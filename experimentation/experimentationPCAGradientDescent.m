% Il seguente script addestra una rete con l'algoritmo discesa del gradiente
% su un training set/validation set estratti casualmente dal dataset MNIST.
% Sul training set viene effettuata la principal component analysis e
% ridotte le dimensioni della colonna della matrice delle immagini. I
% coefficienti ottenuti vengono poi moltiplicati a validation e training
% set. Tali training e validation sono stati estratti in modo da non contenere
...
% hyper-parametri ha raggiunto un accuratezza massima del 91%.
...
EPOCHS = 300;
% Soglia per il numero di colonne da mantenere dopo la PCA sul training
% set.
PCA_THRESHOLD = 0.95;
...
% Applicazione della principal component analysis sul training set.
[coeff, noComponents] = applyPrincipalComponentAnalysis(trainingSetData, PCA_THRESHOLD);
% Moltiplico i coefficienti calcolati durante la principal component
% analysis per ottenere le nuove matrici del training, validation e test
% set.
trainingSetData = trainingSetData * coeff;
validationSetData = validationSetData * coeff;
testSetData = testSetData * coeff;
...
% classi: da 0 a 9).
[neuralNetwork] = newFFMLNeuralNetwork(noComponents, 10, OUTPUT_ACTIVATION_FUNCTION, [struct('layerSize', HIDDEN_NODES, 'activationFunction', HIDDEN_ACTIVATION_FUNCTION)], NETWORK_INF_WEIGHTS, NETWORK_SUP_WEIGHTS);
...
% approccio BATCH e l'algoritmo discesa del gradiente.
[neuralNetwork, trainingSetErrors, validationSetErrors] = trainNetworkGradientDescent(neuralNetwork, trainingSetData, trainingSetLabels, validationSetData, validationSetLabels, EPOCHS, LEARNING_TYPE, ERROR_FUNCTION, ETA, SOFTMAX_FLAG, PRINT_ERROR_FLAG);
